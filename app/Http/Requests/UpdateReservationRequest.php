...
class UpdateReservationRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'reservable_type' => ['sometimes', 'required', 'string', Rule::in(['accommodation', 'conference_room', 'service_only'])],
        ...
        if ($this->input('reservable_type') !== 'service_only' && $this->filled('reservable_type')) {
            $rules['reservable_id'] = 'sometimes|required|integer';
            $rules['checkin_date'] = 'sometimes|required|date';
            $rules['checkout_date'] = 'sometimes|required|date|after_or_equal:checkin_date';
        } elseif ($this->input('reservable_type') === 'service_only' && $this->filled('reservable_type')) {
            $rules['reservable_id'] = 'nullable';
            $rules['checkin_date'] = 'sometimes|required|date';
            $rules['checkout_date'] = 'sometimes|required|date|after_or_equal:checkin_date';
        } else {
            // If reservable_type is not being changed or set, keep existing rules if they exist
            if ($this->filled('reservable_id') || $this->filled('checkin_date') || $this->filled('checkout_date')) {
                $rules['reservable_id'] = 'sometimes|required|integer';
                $rules['checkin_date'] = 'sometimes|required|date';
                $rules['checkout_date'] = 'sometimes|required|date|after_or_equal:checkin_date';
            }
